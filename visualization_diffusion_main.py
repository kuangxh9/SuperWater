...
from utils.sampling import randomize_position, sampling, randomize_position_1, randomize_position_2, \
    randomize_position_multiple, sampling_test1, larger_original_graph
from utils.utils import get_model, get_symmetry_rmsd, remove_all_hs, read_strings_from_txt, ExponentialMovingAverage
from utils.visualise import PDBFile
from utils.water_num_match import match_num_water
...
parser.add_argument('--project', type=str, default='waterbind_inf', help='')
...
parser.add_argument('--cache_path', type=str, default='data/cacheNew_allatoms',
...
parser.add_argument('--split_path', type=str, default='data/splits/timesplit_no_lig_overlap_val',
                    help='Path of file defining the split')
parser.add_argument('--no_overlap_names_path', type=str, default='data/splits/timesplit_test_no_rec_overlap',
...
parser.add_argument('--no_final_step_noise', action='store_true', default=False,
...
parser.add_argument('--resnum_ratio', type=int, default=1, help='')
...
if args.out_dir is None: args.out_dir = f'inference_out_filtered/{args.run_name}'
...
if args.confidence_model_dir is not None:
    if not (confidence_args.use_original_model_cache or confidence_args.transfer_weights):
        ...
        confidence_test_dataset = PDBBind(transform=None, root=args.data_dir, limit_complexes=args.limit_complexes,
                                          ...
                                          matching=not True, keep_original=True,
...
###
# num_metal = 100
# print('The number of added water is ', num_metal)
###

for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
    if confidence_model is not None and not (confidence_args.use_original_model_cache or
                                             confidence_args.transfer_weights) and orig_complex_graph.name[
        0] not in confidence_complex_dict.keys():
        skipped += 1
        print(
    ...
    while not success:  # keep trying in case of failure (sometimes stochastic)
        ...
        num_water = int(res_num * args.resnum_ratio)
        print('\nThe number of added water is ', num_water)
        # randomize_position_1(data_list, args.no_random, score_model_args.tr_sigma_max)
        # randomize_position_2(data_list, args.no_random, score_model_args.tr_sigma_max)  ## works
        randomize_position_multiple(data_list, args.no_random, score_model_args.tr_sigma_max, water_num=num_water)
        larger_original_graph(orig_complex_graph, score_model_args.tr_sigma_max, water_num=num_water)
        ...
        if args.save_visualisation:
            ...
            for idx, graph in enumerate(data_list):
                ...
                ### add more metal ion to position of 0 in order to match up with num_metal
                new_lig = match_num_water(lig, num_water=num_water)
                ###
                pdb = PDBFile(new_lig)
                pdb.add(new_lig, 0, 0)
        ...
        rec_path = os.path.join(args.data_dir, data_list[0]["name"][0][:4],
                                f'{data_list[0]["name"][0][:4]}_protein_processed.pdb')
        if not os.path.exists(rec_path):
            rec_path = os.path.join(args.data_dir, data_list[0]["name"][0][:4],
                                    f'{data_list[0]["name"][0][:4]}_protein_obabel_reduce.pdb')
        ...
        if not args.no_model:
            ...
            data_list, confidence = sampling_test1(data_list=data_list, model=model,
        ...
        if args.save_visualisation:
            if confidence is not None:
                ...
                for rank, batch_idx in enumerate(np.argsort(rmsd)):
                    visualization_list[batch_idx].write(
                        f'{args.out_dir}/{data_list[batch_idx]["name"][0]}_{rank + 1}_{rmsd[batch_idx]:.1f}_{num_metal}.pdb')
