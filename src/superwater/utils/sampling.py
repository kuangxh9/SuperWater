...
def sampling(data_list, model, inference_steps, tr_schedule, device, t_to_sigma, model_args,
             no_random=False, ode=False, save_visualization=False, visualisation_dir='outputs/diff_process', confidence_model=None, confidence_data_list=None,
    ...
    for t_idx in range(inference_steps):
        ...
        for complex_graph_batch in loader:
            ...
            if torch.isnan(tr_score).any():
                print("Warning: NaN encountered in predicted translation score during sampling")
        ...
        if save_visualization:
            ...
            for complex_graph in data_list:
                file_path = f"{visualisation_dir}/water_{complex_graph['name']}_L{inference_steps}.pdb"
