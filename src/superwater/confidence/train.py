...
def train_epoch(model, loader, optimizer, args, mad_prediction):
    model.train()
...
def test_epoch(model, loader, args, mad_prediction):
    model.eval()
...
def train(args, model, optimizer, scheduler, train_loader, val_loader, run_dir):
    ...
    for epoch in range(args.n_epochs):
        logs = {}
        train_metrics = train_epoch(model, train_loader, optimizer, args, args.mad_prediction)
        print("Epoch {}: Training loss {:.4f}".format(epoch, train_metrics['confidence_loss']))
        val_metrics, baseline_metric = test_epoch(model, val_loader, args, args.mad_prediction)
...
def construct_loader_origin(args_confidence, args, t_to_sigma):
    # Like construct_loader, but uses batch_size=1 and a plain DataLoader (not DataListLoader):
    # the score model samples one complex at a time when building the confidence dataset.
...
def construct_loader_confidence(args, device):
    ...
    if exception_flag:
        raise Exception('We encountered the exception during train dataset loading: ' + train_load_error)
...
def main():
    ...
    if args.transfer_weights:
        ...
        checkpoint = torch.load(f'{args.restart_dir}/last_model.pt', map_location=torch.device('cpu'))
