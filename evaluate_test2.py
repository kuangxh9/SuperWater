...
from utils.sampling import randomize_position, sampling, randomize_position_1, randomize_position_2, randomize_position_multiple,sampling_test1, larger_original_graph
...
parser.add_argument('--split_path', type=str, default='data/splits/test.txt', help='Path of file defining the split')
...
parser.add_argument('--actual_device', type=str, default=None, help='')
...
if args.out_dir is None: args.out_dir = f'inference_out/{args.run_name}_rr{args.resnum_ratio}_L{args.inference_steps}'
...
if args.actual_device is None:
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
else:
    device = args.actual_device
...
###
# num_water = 700
# print('The number of added water is ', num_water)
###
...
for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
    ...
    while not success: # keep trying in case of failure (sometimes stochastic)
        ...
        rec_path = os.path.join(args.data_dir, data_list[0]["name"][0][:4], f'{data_list[0]["name"][0][:4]}_protein_processed.pdb')
        if not os.path.exists(rec_path):
            rec_path = os.path.join(args.data_dir, data_list[0]["name"][0][:4], f'{data_list[0]["name"][0][:4]}_protein_obabel_reduce.pdb')
        ...
        if args.save_visualisation:
            if confidence is not None:
                ...
                for rank, batch_idx in enumerate(np.argsort(rmsd)):
                    visualization_list[batch_idx].write(
                        f'{args.out_dir}/{data_list[batch_idx]["name"][0]}_{rank + 1}_{rmsd[batch_idx]:.1f}_{num_water}.pdb')
